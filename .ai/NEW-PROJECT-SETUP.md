# LSC Group — New Project Setup Script
...
Copy `lsc-group/` to your new project's WordPress themes directory and rename it to your project slug:

```bash
# Example
cp -r /path/to/lsc-group /your/wp/wp-content/themes/travelnerds-theme
```

> Do **not** run the script inside the `lsc-group` folder itself. Always work on a copy.
...
> A copy like `wp-content/themes/my-project/lsc-group/style.css` (one level too
...
> ```bash
> mv /your/wp/wp-content/themes/my-project/lsc-group /your/wp/wp-content/themes/travelnerds-theme
...
> (`mosharafmanu/lsc-group`). Committing or pushing from the copy without
...
git init
git add .
git commit -m "Initial copy from lsc-group starter"
```
...
| PHP Function Prefix | `travelnerds` | All `lsc_` → `travelnerds_` function names |
| CSS Token Prefix | `tn` | All `--lsc-` → `--tn-` CSS custom properties and utility classes |
...
| From | To |
|---|---|
| `LSC Group` | Theme Name |
| `lsc-group` | Theme Slug |
| `lsc_` | `{prefix}_` |
| `--lsc-` | `--{css-prefix}-` |
| `bg-lsc-` | `bg-{css-prefix}-` |
| `color-lsc-` | `color-{css-prefix}-` |
...
| File | What changes |
|---|---|
| `style.css` `:root {}` | Token prefix via global `--lsc-` replacement |
...
| `languages/lsc-group.pot` | Renamed to `languages/{text-domain}.pot` |
| `.ai/*.md` | All `lsc_`, `lsc-group`, `--lsc-` references |
...
**Script says Theme Slug cannot be 'lsc-group'**
...
```bash
perl -pi -e 's/lsc-group/your-slug/g' path/to/the/file.php
```
...
git checkout .
```
If you didn't, copy the original `lsc-group` starter again and start fresh.
