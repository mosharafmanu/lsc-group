...
?>
	...
	<main id="primary" class="site-main search-results-page">
		...
		<header class="search-results-header lsc-container layout-padding pt-50 pt-md-70 pt-lg-100">
			<span class="archive-label"><?php esc_html_e( 'Search', 'lsc-group' ); ?></span>
			<h1 class="archive-title">
				<?php printf(
					/* translators: %s: search query */
					esc_html__( 'Results for "%s"', 'lsc-group' ),
					esc_html( get_search_query() )
				); ?>
			</h1>
			<?php if ( $result_count > 0 ) : ?>
				<p class="search-result-count"><?php printf(
					esc_html( _n( '%s result found', '%s results found', $result_count, 'lsc-group' ) ),
		...
		<?php if ( have_posts() ) :
			...
			<?php if ( $products_html ) : ?>
				<section class="search-results-group search-results-group--products lsc-container layout-padding pt-40 pt-md-50">
					<h2 class="search-results-group__title">
						<?php esc_html_e( 'Products', 'lsc-group' ); ?>
		...
		<?php else : ?>
			...
			<div class="search-no-results lsc-container layout-padding pt-50 pb-50 pb-md-70 pb-lg-100">
				<p class="search-no-results__text"><?php printf(
					/* translators: %s: search query */
					esc_html__( 'Sorry, nothing matched "%s". Try a different search term.', 'lsc-group' ),
					esc_html( get_search_query() )
				); ?></p>
				<?php get_search_form(); ?>
			</div>
